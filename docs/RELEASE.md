...
When the ref is a tag such as `v0.1.2`, the workflow attaches `dist/*.zip` and `dist/*.sha256` to the GitHub Release.
...
For local release packaging, the repository builder can call Blender's official ZIP builder before writing `public/index.json`:
