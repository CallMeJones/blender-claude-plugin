...
`capture_animation_playblast` captures sampled viewport PNG frames across an animation range when Blender is running with an interactive window. The metadata resource lists exact frame URIs so external clients can inspect timing, spacing, staging, arcs, and contact poses without relying only on keyframe data. `review_playblast_against_brief` also derives compact pixel digests and frame-to-frame motion deltas from available PNGs, then emits `repair_operations` with executable `tool_call` payloads for deliberate follow-up repairs. `run_animation_repair_loop` can apply a bounded allowlisted subset of those operations, skip under-specified repairs, and re-run review while preserving the preview commit/revert model. In background/headless mode capture fails soft and reports that an interactive window is required.
