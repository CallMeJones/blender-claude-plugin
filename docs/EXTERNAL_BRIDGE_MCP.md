...
`draft_script` is available for custom and larger advanced Blender Python, with a 500k-character normal-script ceiling for procedural scene-building payloads. If a draft overlaps helper-covered work such as transforms, materials, bounded object creation, 2D/storyboard setup, camera moves, or render/camera/world settings, the bridge may return helper advice, but it no longer refuses the script solely for helper overlap. `draft_script` still refuses external asset download/import, project file lifecycle operations, and persistent simulation/cache bakes.

Use `draft_privileged_script` only when a custom external asset or project-file script is genuinely needed. The call must include `script_kind`, an approval summary, declared paths, declared URLs, destructive actions, and complete code. The privileged analyzer can allow declared filesystem/network/project-file APIs, but dynamic code/process escapes remain blocked. Privileged scripts stage in the same pending-script UI, show the manifest in Blender, and never auto-run under normal external script trust; use manual Run or `Approve External` for a fresh one-time token. Declared paths, URLs, and destructive actions are review/audit context for the user, not a runtime filesystem or network sandbox.

For broad advanced work, compact MCP mode exposes `plan_advanced_scene_workflow` and `get_2d_animation_details` directly. Use the planner before custom scripts when a request mixes advanced 3D, 2D/storyboard, animation, simulation, compositor/render, or unclear helper coverage. For storyboard or animatic prompts, route through `get_2d_animation_details`, `create_storyboard_panels`, `create_2d_cutout_layer`, and optionally `create_camera_dolly_animation` or `create_directed_animation_shot`. For procedural 3D stack, kit, or basic modeling prompts, use `create_procedural_object_kit`, `apply_procedural_array_stack`, `edit_mesh`, `curve_to_mesh`, `boolean_op`, `mirror_model`, `symmetrize_model`, `solidify_model`, and geometry-node inspection before custom mesh or node scripts. `edit_mesh` is intentionally bounded to single-operation topology edits on single-user mesh data with live-preview mesh snapshots. For cloth setup, use `add_cloth_simulation_to_selected`, then inspect simulation state before any persistent bake.
...
Use these prompts after installing a fresh zip and refreshing the MCP client. During routing tests, revoke external script trust in Blender so early `draft_script` fallback is visible. Trust mode may run animation-like scripts after static checks pass; helper/workflow guidance should appear as advice, not a hard refusal. Custom asset/project-file scripts should route to `draft_privileged_script`, and persistent-bake paths remain explicit-approval only.
...
| `use a boolean cutter, mirror the model, symmetrize it, and solidify wall thickness` | `plan_advanced_scene_workflow` -> `boolean_op` / `mirror_model` / `symmetrize_model` / `solidify_model` | The client uses Phase 1 non-destructive modeling helpers before custom mesh-edit Python. |
| `extrude mesh faces, inset panels, bridge boundary loops, merge by distance, and convert curve to mesh` | `plan_advanced_scene_workflow` -> `edit_mesh` / `curve_to_mesh` | The client uses bounded Phase 1 mesh-data helpers before custom mesh-edit Python. |
