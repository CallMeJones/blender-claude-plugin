...
- `create_shader_material(name, preset, base_color, metallic, roughness, alpha, emission_color, emission_strength)`
- `add_geometry_nodes_modifier(name, node_group_name, template)`
...
- `plan_director_workflow(prompt, target_objects, deliverables)`
- `plan_advanced_scene_workflow(prompt, domains, target_objects)`
- `plan_asset_import_workflow(prompt, provider, asset_id, uid, target_object_name, presentation_preset)`
...
`get_animation_scene_context` is the first Milestone 7D routing layer. It does not replace deep detail tools; it summarizes likely animation ownership so external agents can choose the right next inspection before editing. It flags rig-driven objects, likely rig control bones, shape-key deformation targets, material or shader animation, object/data/action owners, action slots and keyed channel ranges when available, pose-marker/pose-library candidates, constraints, drivers, NLA tracks, simulation/rigid-body hints, likely contact surfaces, and camera readiness, then returns subject routing with animation-owner recommendations and routing confidence. Its `animation_hardening` block adds risk flags, required pre-mutation inspections, recommended review tools, and repair-loop limits for rig controls, shape keys, material nodes, simulation/cache state, contact/support, and camera framing. Use those hardening entries before replacing existing animation or running bounded repairs; common next tools include `get_rigging_details`, `get_shape_key_details`, `get_material_node_details`, `get_animation_details`, `get_simulation_details`, `inspect_simulation_bake`, `analyze_camera_framing`, and `get_render_camera_compositor_details`.
...
The advanced helpers are intentionally bounded. They create useful starter states and simple edits without exposing arbitrary node graph, rig, compositor, or persistent simulation mutation. `plan_director_workflow` is the read-only orchestration entry point for broad scene, asset, animation, evidence, and preview decision work; it composes inspection, `plan_asset_import_workflow`, `plan_advanced_scene_workflow`, animation workflow/review/repair, visual evidence, and explicit commit/revert decision options without mutating the scene or flattening those decisions into executable calls. `plan_advanced_scene_workflow` is the domain routing entry point for advanced 3D, 2D/storyboard, advanced animation, simulation setup, asset import, compositor/render, and script-heavy requests. `plan_asset_import_workflow` maps external asset requests to discovery, async download/cache, queued import, post-import organization/staging, and viewport evidence instead of synchronous fallback tools.

2D helpers currently cover storyboard/animatic panels and cutout layers. Material helpers include bounded shader presets such as brushed metal, clear glass, emissive accents, rubber black, warm wood, and screen glow. Procedural/modeling helpers cover reversible Geometry Nodes starter templates (`passthrough`, `transform`, `join_geometry`, `set_position`, `subdivide_mesh`), object-kit templates including kitbash, radial/scatter/product, mechanical-joint, control-panel, studio-prop, mechanical-part, modular-wall-panel, and pipe-run generators, non-destructive modifier stacks for procedural arrays, Booleans, mirror, symmetry, and solidify thickness, plus bounded mesh-data edits (`edit_mesh`) for extrude, inset, boundary-loop bridge, dissolve-degenerate, and merge-by-distance on single-user mesh data with snapshot rollback. `curve_to_mesh` creates preview-safe mesh copies from curve/text sources without destroying the original. Asset plans with no known provider or no concrete asset identifier stop after discovery and require choosing a provider plus asset id/uid before download/import/presentation calls are emitted. Directed-shot helpers cover common camera/reveal/path/turntable/crane/truck shots. Compositor support is inspection/planning-first until node-tree rollback exists. When an external agent needs a custom geometry-node network, production rig, compositor graph, broad destructive mesh operation beyond the bounded `edit_mesh` operations, custom provider import/export logic, custom Grease Pencil stroke editing, or complex simulation setup, it should draft approved Python after inspecting context and docs.
